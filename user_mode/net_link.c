...
void nl_init(int port, nl_socket *nl){
    ...
    nl->saddr.nl_family = AF_NETLINK; //AF_NETLINK
    nl->saddr.nl_pid = port;  //端口号(port ID)
...
void nl_recv(nl_socket *nl){
    ...
    nl->nlh->nlmsg_type = 0;
    nl->nlh->nlmsg_seq = 0;
    nl->nlh->nlmsg_pid = (nl->saddr).nl_pid; //self port
...
void nl_close(nl_socket *nl){
    close(nl->skfd);
    free((void *)nl->nlh);
    printf("Attention: net_link port: %d has been released.", nl->port_id);
}
